...
public class UserService implements IUserService {
    ...
    public User buildUserFromUserModelDTO(UserModelWithoutIdDTO userModel) throws UserServiceBuildingException {
        try {
            log.info("The creation of a user with an email: " + userModel.getEmail() + " from person");
            ...
            log.info("The creation of a user with an email: " + userModel.getEmail() + " completed successfully");
            return user;
        } catch (UserServiceValidationException | UserServiceRepositoryException e) {
            log.error(e.getMessage());
            throw new UserServiceBuildingException("Unable to create a user with email: " + userModel.getEmail(), e);
        }
    }
